...
const UpdateSessionModal = ({ session, onClose }: Props) => {
    ...
    useEffect(() => {
        axios.get('http://localhost:5173/api/sessions/availability')
    ...
    const handleDelete = async () => {
        ...
        try {
            await axios.delete(`http://localhost:5173/api/sessions/delete/${session.id}`);
    ...
    const handleConfirm = async () => {
        try {
            ...
            await axios.post(
                `http://localhost:5173/api/sessions/update/${session.id}`,
                payload
            );
